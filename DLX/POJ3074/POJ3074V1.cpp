...
DLX g;
bool hashn[10][10], hashm[10][10], hashp[10][10], hash_[10][10];
char str[100];
int main()
{
    while(scanf("%s",str) && str[0] != 'e')
    {
        ...
        for(int i = 0;i < 9;++i)
            for(int j = 0;j < 9;++j){
                if(str[i*9+j] != '.')
                {
                    ...
                    hashn[i][t] = true;
                    hashm[j][t] = true;
                    hashp[i/3*3+j/3][t] = true;
                    g.ans[i][j] = str[i*9+j] - '0';
        ...
        for(int i = 0;i < 9;++i)
            for(int j = 0;j < 9;++j){
                ...
                {
                    for(int k = 1;k <= 9;++k)
                        if(!hashn[i][k] && !hashm[j][k] && !hashp[i/3*3+j/3][k])
                        {
                            g.Link(++row, i*9+j+1);
                            g.Link(row, 81+i*9+k);
                            g.Link(row, 162+j*9+k);
                            g.Link(row, 243+(i/3*3+j/3)*9+k);
